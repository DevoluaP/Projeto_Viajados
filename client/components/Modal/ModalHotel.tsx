...
type ModalHotelProps = {
  visible: boolean;
  hotel: any;
  onClose: () => void;
  imagemHotel?: ImageSourcePropType;
};
...
const ModalHotel = ({
  ...
  const reservarHotel = async () => {
    try {
      ...
      if (!dateRegex.test(dataEntrada) || !dateRegex.test(dataSaida)) {
        Alert.alert(
          "Erro",
          "Por favor, insira as datas no formato DD/MM/YYYY (ex.: 10/04/2025)."
        );
        return;
      }
      ...
      if (response.ok) {
        ...
        Alert.alert(
          "Erro",
          result.message ||
            `Erro ao reservar hotel (Status: ${response.status})`
        );
      }
    } catch (error) {
  ...
  const valorTotal = diasEstadia
    ? (hotel.preco_diaria * diasEstadia).toFixed(2)
    : null;
                ...
                <View style={styles.detailsHeader}>
                  <View style={styles.ratingContainer}>
                    {Array.from({ length: 5 }).map((_, i) =>
                      renderStar(i, hotel.avaliacao)
                    )}
                  </View>
                  <View style={styles.priceTag}>
                    <Text style={styles.priceLabel}>Diária</Text>
                    <Text style={styles.priceValue}>
                      R$ {parseFloat(hotel.preco_diaria).toFixed(2)}
                    </Text>
                  </View>
                </View>
                    ...
                    <View style={styles.dateInput}>
                      ...
                      <View style={styles.dateInputContainer}>
                        ...
                        <TextInput
                          ...
                          value={dataEntrada}
                          onChangeText={(text) =>
                            formatarDataInput(text, setDataEntrada)
                          }
                    ...
                    <View style={styles.dateInput}>
                      ...
                      <View style={styles.dateInputContainer}>
                        ...
                        <TextInput
                          ...
                          value={dataSaida}
                          onChangeText={(text) =>
                            formatarDataInput(text, setDataSaida)
                          }
                          placeholder="DD/MM/AAAA"
                          placeholderTextColor="#999"
                          keyboardType="numeric"
